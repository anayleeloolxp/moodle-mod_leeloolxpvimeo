...
if ($searchstring) {
    ...
    $leeloolxpvimeos = $DB->get_records_sql('SELECT v.*, c.fullname as coursename FROM {leeloolxpvimeo} as v left join {course} as c on c.id = v.course WHERE '.$DB->sql_like('v.name', ':name', false, false).' '.$sortbysql, ['name' => '%'.$DB->sql_like_escape($searchstring).'%'], $from, $perpage);

    $leeloolxpvimeoscount = $DB->get_record_sql('SELECT count(*) as total FROM {leeloolxpvimeo} as v left join {course} as c on c.id = v.course WHERE '.$DB->sql_like('v.name', ':name', false, false).' '.$sortbysql, ['name' => '%'.$DB->sql_like_escape($searchstring).'%']);

}else{

    $leeloolxpvimeos = $DB->get_records_sql('SELECT v.*, c.fullname as coursename FROM {leeloolxpvimeo} as v left join {course} as c on c.id = v.course'.' '.$sortbysql, [], $from, $perpage);

    $leeloolxpvimeoscount = $DB->get_record_sql('SELECT count(*) as total FROM {leeloolxpvimeo} as v left join {course} as c on c.id = v.course'.' '.$sortbysql);
...
$leeloolxpextras = $DB->get_records_sql('SELECT v.*, c.fullname as coursename FROM {leeloolxpvimeo} as v left join {course} as c on c.id = v.course '.$sortbysql.' limit 12');
